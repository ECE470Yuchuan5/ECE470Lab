...
class particleFilter:
	...
	def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
		...
		self.particles = [[0 for x in range(2)] for y in range(numParticles)]
		self.weights = [[0 for x in range(2)] for y in range(numParticles)]
	   
		for i in range(numParticles):
			self.particles[i] = [np.random.uniform(self.curMin[0],self.curMax[0]),
								np.random.uniform(self.curMin[1],self.curMax[1])]
			self.weights[i] = 1/numParticles        
	...
	def Measurement_Model(self, x, y):
		...
		n = self.numParticles
		rPos = self.model.readingSensor(x, y)
		pPos = [None] * n
		w_t = [None] * n
		s = 0
		for i in range(n):
			pPos = self.model.readingMap(self.particles[i])
			distance = np.sqrt((rPos[0]-pPos[0])**2 + (rPos[1]-pPos[1])**2 + (rPos[2]-pPos[2])**2 + (rPos[3]-pPos[3])**2)
			w_t[i] = np.exp(-distance**2/(2*self.std))
			s += w_t[i]
		if(s==0):
			s = 1
		w_t = w_t/s
	...
	def calcPosition(self):
		...
		prody = [None] * n
		for i in range(n):
			prodx[i] = self.particles[i][0]*w[i]
			prody[i] = self.particles[i][1]*w[i]
		estx = sum(prodx)
		esty = sum(prody)
	   
		if(estx > self.curMax[0]):
			estx = self.curMax[0]
		if(estx < self.curMin[0]):
			estx = self.curMin[0]
		   
		if(esty > self.curMax[1]):
			esty = self.curMax[1]
		if(esty < self.curMin[1]):
			esty = self.curMin[1]
